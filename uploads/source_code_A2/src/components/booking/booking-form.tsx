...
export function BookingForm({
  ...
  const calculateTotalPrice = () => {
    return tourPrice * Number.parseInt(formData.participants);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Gửi dữ liệu đến API endpoint thực sự
      const response = await fetch("/api/bookings", {
        ...
        body: JSON.stringify({
          ...
          paymentMethod: "CREDIT_CARD", // Mặc định, sẽ được chọn ở trang thanh toán
          status: "PENDING",
        }),
      });
      ...
      if (!response.ok) {
        throw new Error(data.error || "Đã xảy ra lỗi khi đặt tour");
      }

      const booking = data.booking;
      setFormSubmitted(true);

      // Chuyển hướng đến trang thanh toán với ID booking thật từ cơ sở dữ liệu
      setTimeout(() => {
