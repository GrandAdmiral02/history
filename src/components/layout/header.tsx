...
export function Header() {
  ...
  return (
    ...
    >
      <div className="container flex items-center justify-between py-3 px-4 sm:px-6 md:px-8 max-w-7xl">
        {/* Logo Section */}
        ...
        <div className="flex items-center gap-3 md:gap-4">
          {/* Search Button */}
          <SearchButton
            className="hidden md:block text-white hover:text-amber-100 transition-colors duration-300 focus:outline-none focus:ring-2 focus:ring-amber-300 rounded-full"
          ...
          <NavigationMenu className="hidden md:block">
            <NavigationMenuList className="flex gap-2">
              <NavigationMenuItem>
                <Link
                  href="/"
                  className="group inline-flex h-10 w-max items-center justify-center rounded-md bg-transparent px-4 py-2 text-sm font-medium text-white hover:bg-amber-200/20 hover:text-amber-100 focus:bg-amber-200/20 focus:text-amber-100 focus:outline-none focus:ring-2 focus:ring-amber-300 transition-all duration-300"
                >
                  Trang Chủ
                </Link>
              </NavigationMenuItem>
              <NavigationMenuItem>
                <NavigationMenuTrigger
                  className="text-sm font-semibold text-white bg-teal-800/40 hover:bg-teal-800/60 hover:text-amber-100 focus:bg-teal-800/60 focus:text-amber-100 focus:outline-none focus:ring-2 focus:ring-amber-300 transition-all duration-300 px-4 py-2 rounded-md"
                  aria-controls="destinations-menu"
                >
                  Điểm Đến
                </NavigationMenuTrigger>
                <NavigationMenuContent>
                  <ul className="grid w-[350px] gap-3 p-4 md:w-[500px] md:grid-cols-2 lg:w-[600px] bg-white/95 backdrop-blur-lg rounded-xl shadow-md">
          ...
          <Button
            variant="ghost"
            size="icon"
            className="md:hidden h-9 w-9 rounded-full text-white hover:text-amber-100 hover:bg-teal-800/60 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-amber-300"
            ...
            {session?.user ? (
              ...
              <Link href="/login">
                <Button
                  variant="default"
                  size="sm"
                  className="bg-gradient-to-r from-teal-500 to-amber-500 text-white hover:from-teal-600 hover:to-amber-600 transition-all duration-300 transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-amber-300 rounded-md text-sm"
                >
                  Đăng nhập
                </Button>
              </Link>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden bg-gradient-to-br from-teal-600 via-cyan-500 to-amber-100 border-t border-teal-300/50 shadow-lg">
          <div className="container py-4 px-4 sm:px-6 flex flex-col gap-3">
            <Link
              href="/"
              className="text-sm font-medium text-white hover:text-amber-100 hover:bg-teal-800/60 px-3 py-2 rounded-md transition-all duration-300"
              onClick={toggleMobileMenu}
            >
              Trang Chủ
            </Link>
            <div className="relative">
              <button
                className="w-full text-left text-sm font-medium text-white hover:text-amber-100 hover:bg-teal-800/60 px-3 py-2 rounded-md transition-all duration-300"
                aria-expanded="true"
              >
                Điểm Đến
              </button>
              <ul className="mt-2 pl-4 flex flex-col gap-2">
                {components.map((component) => (
                  <li key={component.title}>
                    <Link
                      href={component.href}
                      className="block text-sm text-white hover:text-amber-100 hover:bg-teal-800/60 px-3 py-1.5 rounded-md transition-all duration-300"
                      onClick={toggleMobileMenu}
                    >
                      {component.title}
                    </Link>
            ...
            <div className="flex gap-4 mt-4">
              {[
                ...
                <a
                  ...
                  href={href}
                  aria-label={label}
                  className="text-white hover:text-amber-100 transition-colors duration-300"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="18"
    ...
    <li>
      <NavigationMenuLink asChild>
        <Link
          ...
          {...props}
        >
          <div className="text-sm font-medium leading-none text-teal-900">{title}</div>
